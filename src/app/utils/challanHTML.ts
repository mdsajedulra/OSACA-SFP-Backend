import { toBangla } from "./toBaangla";
...
export function challanHTML(
  ...
  const sentRows = allFoods
    .map(
      (food, i) => `
    <tr>
    <td style="text-align:center">${toBangla(i + 1)}</td>
    ...
    ${food.slice(0, 3) === c.foodName.slice(0, 3) ? toBanglaNumber(c.quantity) : ""}
    </td>
    <td></td>
    </tr>`,
  )
  ...
  <div class="serial form-badge " style="width:50%">${toBangla(Number(c.pdOfficeSerial))}/</div>
  <div class="challan-no ">চালান নম্বর: ${toBanglaNumber(Number(c.challanNo))}</div>
  </td>
  ...
  <div class="frow"><span class="fl">EMIS কোড</span><span class="fv">: ${toBanglaNumber(Number(c.schoolCode))}</span></div>
